...
def verify_token(token):
    try:
        data = jwt.decode(token, key=f'{secret_key}', algorithms=['HS256'])
        user_id = data["sub"]
...
class User(db.Model):
    ...
    def encode_token(self):
        ...
        try:
            return jwt.encode(payload, key=f"{secret_key}", algorithm="HS256")
        except Exception as ex:
            raise ex
...
class ClothesResource(Resource):
    ...
    def get(self):
        return UserOutSchema().dump(auth.current_user())
